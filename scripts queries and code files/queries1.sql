...
select top 25 count(order_id) as count_of_order,sum(order_value) as sum_of_order
from orders
...
select  (select cast((count(user_id)) as float) from orders)  / (select cast((count(user_id)) as float) from users ) 
